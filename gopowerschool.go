...
func (client *PublicPortalServiceJSONPortType) GetStudent(username, password string) (*StudentDataVO, error) {
	...
	if response.Return_.MessageVOs != nil {
		return nil, fmt.Errorf("error: %s - %s", response.Return_.MessageVOs[0].Title, response.Return_.MessageVOs[0].Description)
	}
	...
	studentDataArguments := GetStudentData{UserSessionVO: &newSession, StudentIDs: []int64{int64(response.Return_.UserSessionVO.StudentIDs[0])}, Qil: &QueryIncludeListVO{Includes: []int32{1}}}
